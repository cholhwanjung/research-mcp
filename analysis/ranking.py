...
def render_sorted_list(
    ...
    """sort='count'(기본)·'velocity'. velocity 모드는 줄에 vel 수치도 포함."""
    if sort == "velocity":
        if current_year is None:
            from datetime import datetime, timezone

            current_year = datetime.now(timezone.utc).year
        sorted_papers = sort_by_velocity(papers, current_year, bias_newcomer)[:top_k]
    else:
    ...
    lines = [
        ...
        + (f" (sort=velocity, year={current_year})" if sort == "velocity" else ""),
        "",
    ]
