---
name: daily-digest
description: HF Daily Papers를 받아 `digests/{date}.md`(YYYY-MM-DD 형식) 노트로 vault에 누적한다.
trigger:
  - "오늘 신착 정리해"
  - "daily digest"
  - "오늘 트렌딩 논문"
...
| # | Tool | 목적 |
|---|---|---|
| 1 | `get_hf_daily_papers(date, limit=10)` | HF Daily Papers (비공식 API → 실패 시 HTML fallback) |
...
wikilink 형식 규약:
- **link target**은 제목을 사람-가독 slug로 변환한 값 (예: `minwm`, `qwen-vla`) — 그래프 view에서 노드 텍스트로 표시됨.
- arxiv_id가 아닌 slug를 사용하는 이유: 그래프에서 `2605.30161`보다 `why-far-looks-up`이 직관적.
...
paper-ingest 시 같은 slug로 폴더 + 파일 생성되면 자동 backlink 연결.
...
## Failure handling
- step 1 HF Daily 비공식 API 4xx → 자동으로 HTML fallback. 둘 다 실패하면 빈 결과 — body에 "오늘 HF 응답 없음" 명시.
