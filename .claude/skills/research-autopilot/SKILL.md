...
**거부된 tick도 공짜가 아니다** — 그 시점엔 0 토큰이어도 tick 프롬프트(이 파일 전체)가 대화에 남아 회복 첫 turn에 한꺼번에 들어온다(실측: 2시간 18분 대기의 tick 29회 → 71만 토큰). 워커 1회는 5~10분(실측 2026-09-06, n=10, 평균 7.8분)이고 워커 실행 중 tick은 쌓이지 않고 하나만 대기하므로, 10분이면 반복 사이 공백이 몇 분이고 한도 대기 2시간의 tick도 14회 × 이 파일 크기에 그친다. 그래서 이 파일은 디스패처 분량만 두고 워커 절차는 `WORKER.md`에 — **이 파일을 키우지 않는다.**
