---
name: self-improve
description: 세션 회고를 입력으로 CLAUDE.md / docs/* diff를 제안하고, 사용자 승인 후 저장 + vault changelog에 기록한다 (ADR-008).
trigger:
  - "self-improve"
  - "회고 반영해줘"
  - "이번 세션 정리해서 룰로 만들어"
...
| 5 | diff 제안 — 추가/수정 라인을 명시. 새 ADR이면 다음 ADR-N 번호 자동 추정. | (LLM) |
...
**Summary**: Phase 시작 전 결정 확정 패턴을 워크플로우에 명시.

**Files**:
- CLAUDE.md (워크플로우 §3 라인 추가)
- docs/ADR.md (ADR-013 추가)

**회고 출처**: 사용자 정정 2건 — "생각한 내용은 문서에 반영하지 말고 나랑 상의" (Phase 2 → 3 전환), "권장대로 가줘" (D-1~D-12 확정 시).
```
...
새 ADR 후보는 단순 추가가 아니다:
1. 기존 ADR 목록에서 충돌(supersede) 후보 확인.
2. supersede면 옛 ADR에 `Status: Superseded by ADR-N` 라인만 추가 (본문 불변, [ADR](../../docs/ADR.md) 룰).
3. 새 ADR-N에 Context / Decision / Reasoning / Tradeoffs 네 섹션 모두 채움.
...
## 후속 호출 제안
- 자가개선 누적이 잦으면: `/loop` 스킬로 야간 자동 회고 (옵션 — 별도 ADR로 검토).
- 큰 구조 변경이면: 같은 PR에서 `docs/ARCHITECTURE.md`도 갱신.

## 참고
- [ADR-008 자가개선 거버넌스](../../docs/ADR.md#adr-008)
- [CLAUDE.md 자가개선 표](../../CLAUDE.md)
- harness 5단계: [.claude/commands/harness.md](../../commands/harness.md)
